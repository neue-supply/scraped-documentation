...
In the scene you define in your YAMLYAML is a human-readable data serialization language. It is used to store and transmit data in a structured format. In Home Assistant, YAML is used for configuration, for example in the `configuration.yaml` or `automations.yaml` files.[ [Learn more]](https://www.home-assistant.io/docs/configuration/yaml/) files, please ensure you use all required parameters as listed below.
